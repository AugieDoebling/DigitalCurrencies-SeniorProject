...
def create_rolling_sums(df,times):
    ...
    for i in range(len(times)):
        ...
        df[('sum' + str(times[i]) + '_avg_sentiment')] = df['sentiment'].rolling(times[i]-1).sum().fillna(method = 'bfill')
        ...
        if (i > 1):
            ...
            df[('sum' + str(times[i]) + '_avg_sentiment')] = df[('sum' + str(times[i]) + '_avg_sentiment')] -df['sentiment'].rolling(times[i-1]-1).sum().fillna(method = 'bfill')
    return df
...
def calc_model_variables(tweets):
    ...
    # one hot encode weekday values
    ag['Monday']    = (ag['datetime'].dt.dayofweek == 0).astype(int)
    ag['Tuesday']   = (ag['datetime'].dt.dayofweek == 1).astype(int)
    ag['Wednesday'] = (ag['datetime'].dt.dayofweek == 2).astype(int)
    ag['Thursday']  = (ag['datetime'].dt.dayofweek == 3).astype(int)
    ag['Friday']    = (ag['datetime'].dt.dayofweek == 4).astype(int)
    ag['Saturday']  = (ag['datetime'].dt.dayofweek == 5).astype(int)
    ag['Sunday']    = (ag['datetime'].dt.dayofweek == 6).astype(int)
...
def determine_expected_price(data, model):
    ...
    # fuck it
    data['avg_sentiment'] = data['sentiment']
